...
## Key Fix (2026-08-14) �?Q5_0 negative-d sign bug, S48→int32 truncation, and Q8 first-run corruption (still open)
...
### Bug 1: Q5_0 core `f16_decode` ignores the f16 sign bit �?~50% of real weight blocks sign-flipped
...
**Why it was masked:** the golden model (`sim/golden_model.hpp` `f16_decode_s2416`), the board golden (`linux/tmac_linux.c` `f16_decode_s2416`), and every hardware/golden test used `d=1.0` (positive) �?the sign path was never exercised. The old sim path (`fpga_sim.hpp` `axi_vecmul_tile_q5_0_8x896_axilite`) used sign-respecting `read_f16`, which is why the pre-golden baseline matched CPU at maxdiff 0.0046 while the golden-based sim showed 2�?0.

**Evidence:** `[DIAG row0] exact=0.036140 golden=-0.021725` �?the golden sign-flipped row 0; `blk.0.attn_q` block 0 has `d=0x986B` (bit 15 set, = �?.002157). Confirmed against llama.cpp's `quantize_row_q5_0_ref` (`d = max / -16`, `id = 1/d`, `q = MIN(31, x*id + 16.5)`).
...
**Verification:** new negative-d self-test in `sim/test_golden_model.cpp` (±1.0/±0.5/±0.25, �?, full negative-d tile); new iVerilog regression `verilog/tb_q5_negd.v` (direct `dbg_d_fp`/`dbg_d_pre` check, incl. S16 clamp ±65536→�?2768) wired into the Makefile `sim_q5_negd`. Sim `--compare` Q5_0 maxdiff dropped 2�?0 �?0.0001�?.02. **On silicon:** `--trace` layer norms match the sim (L0=17.49…L23=55.92); every `[q5 acc=gold]` bit-exact.

### Bug 2: S48 accumulator cast to `int32_t` in board output scaling �?rows > 2³¹ wrapped

**Root cause (host):** `linux/tmac_linux.c` scaled results with `(float)(int32_t)raw_s` for both Q8 (`fpga_q8_tile`) and Q5 (`fpga_q5_tile`). The core accumulators are S48; real row sums reach ~8×10�?(e.g. `y�?0 �?raw = 10·65536/0.000082 �?8e9 > 2³¹`), so the `(int32_t)` cast wrapped. The sim used `(float)out[i]` (int64), which is why the sim matched CPU but the board showed maxdiff 10�?7 on Q5 tensors.

**Fix:** cast the S48 value directly to float: `y[row0+i] += (float)raw_s * x_scale * row_scale[i];` (Q8) and `/ 65536.0f` (Q5). Rebuilt `tmac` �?md5 `F5EDAA...`. Verified on silicon: Q5 maxdiff collapsed 10�?7 �?0.0001�?.02.

### Bug 3: Q8 core first run after a Q5 descriptor corrupts rows 16-63 - STALE `sc_burst_done` SCALE-BURST SKIP (ROOT CAUSE FOUND + FIXED IN RTL 2026-08-16)

**Symptom:** `[q8 RAWDIFF]` on scattered rows (24,28,42,47,51,52,55,56...) **only in the first 64-row tile** (rows 0-63) of each `attn_v`; tile 1 (rows 64-127) always clean. Golden correct (host compare: board `q8_golden_raw` == sim `q8_tile_golden`, 0 mismatches on real data), core arithmetic bit-exact (cosim `tb_cosim.v` PASS on real tiles). So the bug is **not** data, golden, or core arithmetic.

**Decisive diagnostic (on-board `--compare` Q8DBG):** the *identical* descriptor re-run comes back **clean** (`max diff=0`). Per-group isolation (num_groups=1..14) confirmed `grp14` (full) == golden exactly while the first-ever run was corrupted. Conclusion: **the first Q8 descriptor processed after a Q5_0 descriptor corrupts rows 16-63 (acc-bank groups g=2..7); the first Q8 compute itself clears the state.** Re-triggers on every Q5->Q8 transition (each `attn_v` follows `attn_q`/`attn_k`).

**Root cause (FSM stale-state, NOT uninitialized LUTRAM):** the Q5_0 path sets `sc_burst_done <= 1` on the **last block`s** `rd_done_rise` in `Q5_BLOCK_COMPUTE_W` (hp_fsm_top.v:1137-1140) and then runs `Q5_READ_RES -> WRITE_RES -> WRITE_RES_W -> DONE` for the final tile, **none of which consume `sc_burst_done`** (Q5_LOAD_NORM clears it only on the *next* tile`s entry). The flag therefore stays 1 into the next descriptor. In the Q8 descriptor`s first `LOAD_SCALES_W`, the condition `sc_burst_done && !rd_unpack_active` is true **on the very first cycle** - so the FSM consumes the flag immediately, *before the first scale burst has even arrived*, decrementing `sc_remaining` 256->192 and issuing `LOAD_SCALES` for DDR offset 64. That re-`rd_start` is **dropped by the single-flight read master** (still busy with the offset-0 burst the FSM had already started). Net effect: only 3 of the 4 scale bursts are actually unpacked, at offsets 0 / 128 / 192 (offset 64 lost):
  - `smem[0..31]`  = bytes 0-63   (rows 0-15,  g0-1: **correct**)
  - `smem[32..63]` = bytes 128-191 (rows 16-31, g2-3: shifted scales - wrong)
  - `smem[64..95]` = bytes 192-255 (rows 32-47, g4-5: shifted scales - wrong)
  - `smem[96..127]` = 0 (reset)   (rows 48-63, g6-7: dequant to 0)
This matches the board `[q8 RAWDIFF]` exactly (rows 16-63, g6-7 zero). The `INIT=0` LUTRAM change (2026-08-16) was a **red herring** - it never fixed the board because the bug is in the FSM handshake, not the core LUTRAM init.

**Why not reproduced in sim (before 2026-08-16):** `tb_hw_fsm_comprehensive.v` `ddr_mem` was only **4 MB** (`[0:524287]`, covers 0x00000000-0x003FFFFF) but Test 10 placed the Q5 descriptor/weights/acts at **0x00400000-0x00412000** - out of bounds, so every Q5 read returned X and the descriptor`s `tensor_type` (X) **fell through to the Q8 default dispatch**: the Q5 path never ran, `sc_burst_done` was never left stale, Test 10 passed. Growing the model to **8 MB** (`[0:1048575]`) lets the Q5 descriptor dispatch for real; Test 10 then **fails on rows 16+** with exactly this pattern.

**Fix (validated in sim):** `sc_burst_done <= 0;` added to the Q8 FETCH_DESC dispatch (`vivado_integration/rtl/hp_fsm_top.v`, ~line 762). `tb_hw_fsm_comprehensive` 10/10 + `tb_hp_fsm_q5_0` 10/10 PASS with the fix; Test 10 FAILs (rows 16+) without it. The warm-up workaround in `fpga_q8_tile` (`linux/tmac_linux.c`) and the `--nowarmup` flag were removed 2026-08-17 and tmac rebuilt; the fix was verified on silicon (2026-08-17: `--compare` shows no `[q8 RAWDIFF]`, layer norms match the sim).

### Sim re-wiring (funnel principle)

`sim/tmac_gguf.cpp` `matmul_fpga_q5_0` now calls `golden::q5_tile_golden` (was the idealized FP32 `axi_vecmul_tile_q5_0_8x896_axilite`), and `matmul_fpga_q8` now calls `golden::q8_tile_golden` (was `axi_vecmul_tile_q8`). This makes the sim reflect the real RTL fixed-point behavior, which is how Bugs 1�? were exposed and how Bug 3 was ruled in/out of the model.

### Open items

- **Q8 residual on 4 layers (blk.1/7/8/13):** `attn_v` maxdiff 0.0109–0.0157 on these layers vs 0.0004–0.004 for the rest, **present in both the warm-up and init-fix runs** — a separate, data-dependent issue (likely a Q8 scale/row-normalization precision effect), not the Q5→Q8 stale-state bug. Worth a follow-up look.
...
## Key Fix (2026-08-13) �?Linux Q6_K dequant: two bugs caused layer-0 explosion on hardware

**Root cause of the layer-0 `ffn_down` explosion** (hidden norm �?115M instead of ~17) that broke inference on the board: two independent bugs in `tmac_linux.c`'s Q6_K dequant path. Both were masked by the fact that all prior testing compared Linux-vs-sim on the SAME shared helpers, or used uniform data.

### Bug 1: Q6_K ql/qh/scale reads missing the block offset `bo`

`tmac_linux.c` Q6_K branch (old lines 245-250) added `bo` ONLY to the super-scale read (`d+bo+208`); the ql/qh/scale byte reads used absolute offsets (`d[lo]`, `d[128+...]`, `d[192+...]`) with no `+bo`. So for every block �?1 (idx �?256), the dequant read **block 0's** ql/qh/scales while using the current block's super-scale �?producing garbage weights. The sim (`tmac_gguf.cpp:326-362`) adds `base` everywhere. Q4_K (lines 252-262) already included `bo` correctly.
...
### Bug 2: `f16_to_f32` unsigned-overflowed negative subnormals �?+256.0

The old subnormal branch was `((sign?-1:1)*mant/1024.0f*0.00006103515625f)` with `mant` a `uint32_t`. `(sign?-1:1)` is an `int`; `int * uint32` promotes the `-1` to unsigned and **wraps**: `0xFFFFFFFF * 172` mod 2³² ÷ 1024 × 2⁻¹⁴ �?**+256.0** instead of `-1.03e-5`. Real Q6_K super-scales ARE negative subnormals (e.g. block 1 of `blk.0.ffn_down.weight` = `0x80ac`), so layer-0 weights were amplified ~2.5e7× �?norm 115M. The sim (`tmac_gguf.cpp:222`) uses a float ternary `(sign ? -1.0f : 1.0f)` so it never wraps �?another reason the sim stayed healthy.
...
- `linux_repro.c` (verbatim Linux forward-pass on x86): layer-0 `fout` norm dropped from **165,461,015 �?17.49**, and ALL 24 layer norms now match the sim's `--dump-layers` (L0=17.49, L1=31.49, L2=812.22, L22=37.47, L23=55.92). Before the fix it reproduced the board's 115,599,718.70 exactly, proving the bug was code, not ARM/FPGA.
- Rebuilt static ARM binary (`arm-linux-gnueabihf-gcc -std=gnu11 -O2 -static`) �?deployed to `linux/boot/tmac` (468,684 B, md5 298c00f147f0506775a5eb1e9908bc74; **superseded 2026-08-14** by the generation-protocol fix �?see the boot-file table below for the current md5).
- **Bare-metal port (`tmac_baremetal.cpp`) had Bug 1 too** �?its Q6_K `lo`/`ho`/`sco` also lacked `+bo` (its `f16_to_f32` in `tmac_baremetal.h` was already the float-ternary form, so Bug 2 absent). Fixed `lo/ho/sco` �?`+bo`, compile-checked with clang (`--target=armv7a-none-eabi`, COMPILE OK).

**2026-08-13 follow-up:** hardware re-test done �?deployed `linux/boot/tmac` on the SD card, ran `./tmac model.tmac --trace` (all 24 layer norms now match the sim: L0=17.49 �?L23=55.92; the earlier 115M explosion is gone). Note: full-token generation still shows an FPGA-vs-CPU mismatch (see `--trace` comparison) �?that is the **separate, pre-existing Q5_0/Q8_0 precision issue** (the Q6_K/Q4_K `ffn_down` matmul runs on the CPU in both paths), not the dequant bug fixed above.
...
**Refinement (2026-08-13, current working tree):** the anticipation was made pipeline-accurate �?smem/act go through an extra Stage-0 register (1 more cycle latency than wmem), so their addresses are anticipated **2 iterations ahead** (column k+1 when g�?, i.e. one wrap away) while wmem is anticipated 1 iteration ahead. `pre_g`/`pre_k`/`pre_valid` register the anticipated (g,k) as pipeline meta, and CLEAR_ACC arms the very first entry's smem/act reads at `acc_clr_cnt==6`. Current form:
```verilog
...
if (g == 7) begin
    ...
    pre_smem_addr <= {3'd0, (k + 6'd1) >= 32};  // (0, half of k+1) �?wrap boundary
    pre_act_addr <= k + 6'd1;
end else begin
    pre_wmem_addr <= {g + 3'd1, k};
...
The Q8 core's `res_dout` was a **registered** output (`res_dout_r <= acc_bX[res_addr]` in an `always @(posedge clk)` block), introducing a 1-cycle latency. The FSM's `READ_RES` captures `act_buf[idx] <= res_dout` in the same cycle it advances `res_addr`, so the readback was systematically shifted by one row: `result[0]` = stale, `result[i]` = `acc_bX[i-1]` for i�?.
...
### Bug 5: Q8 multi-group was a TEST bug, not an RTL bug �?RESOLVED (2026-08-13)
...
the col-pattern weight `(int8_t)(g*64+c+1)` overflows INT8 for `g �?2` (wraps
past 127), so the "expected 401856" was wrong and the weights were garbage. The
actual value 7616 = Σ(int8_t)(c+1) for c=0..895 is the *correct* result for the
wrapped weights.
...
`896·(r+1)`. Also verified in iVerilog (Test 9, 14 groups, row-pattern �?PASS).
...
(`READ_RES_ACC` �?`COPY_ACC_TO_BUF` �?`WRITE_RES`) is correct.
...
8. **Q5_0 block-streaming redesign (2026-07-06):** Replaced 4-cycle/element pipeline (7170 cycles) with block-at-a-time architecture (1904 core cycles + DDR overhead �?856 system cycles/tile). Precomputes d_pre = f16_decode(d) × scale >> 8 once per block, then 1 MAC/cycle per element. Scale indexing bug fixed (core1 used scale[0:1] instead of scale[2:3]). All 8 unit tests + 4 HP FSM dispatch tests PASS (pre-redesign testbench; core unit tests need port-renamed rebuild for new `qs_word`/`blk_num` interface).
...
12. **Bitstream rebuild (2026-07-12, SUPERSEDED by #24):** Vivado build completed. WNS=-0.360 ns (4 failing endpoints). BRAM18: 33�?0 (Q5_0 clean-slate rewrite eliminated 14, Q8 smem/act LUTRAM saved 9). Slice LUTs: 9,898�?,066. LUT as Memory: 97�?77. Slice Regs: 12,781�?4,052. DSP48E1: 22�?3. **Note: this 2026-07-12 bitstream lacked SDIO0 (PCW_EN_SDIO0=0). The frozen bitstream (2026-08-04, #24) adds SDIO0 and passes all HW tests �?the resource numbers below remain current for the PL fabric.**
...
13. **Q8 col_group stale in FETCH_DESC dispatch �?Test 10 RTL fix (2026-07-16):** `col_group` was not reset in the Q8 FETCH_DESC dispatch path (`hp_fsm_top.v:689`). After a multi-group descriptor left `col_group=1`, the next descriptor in the chain loaded weights/scales/acts from wrong DDR offsets. Caused Test 10 tile 0 to fail. Fix: `col_group <= 0` added to Q8 dispatch. Same class as CPU_OP col_group bug (Key Decision #10). All 10 comprehensive HW tests now PASS.
...
- `REG_Q8_DEBUG[25]` (q8_done) transitions 0�? when computation completes; this is the most important signal for debug
- `REG_Q8_DEBUG[16:11]` (q8_core_k) should count from 0�?3 during result readback; if stuck at 0, the FSM never entered READ_RES/READ_RES_ACC
...
**Q8 BRAM waste analysis and smem/act �?LUTRAM conversion (2026-07-07):** Analyzed `matmul_q8_core.v` BRAM utilization and found 9 BRAM18 held arrays using < 5% of their declared capacity:
    ...
    **Result:** Q8 core BRAM drops from 17�? (wmem only). Total system BRAM ~10 (Q8 8 + Q5_0 2). LUTRAM increases ~128 (smem) + 16 (act) = +144 LUTRAM (3.3% of 4,400 capacity, well within budget). All 123 core simulation tests PASS: Q8 6/6, Q4K 4/4, Q6_K 97/97, HP FSM 7/7, HP FSM Q5_0 9/9. INT16 smoke pre-existing fail (unrelated wmem addressing). Q5_0 standalone testbench pre-existing compile error (old port interface, not updated for per-block rewrite).

## Current Status (2026-08-17) - Q5 sign bug + S48 int32 truncation fixed; Q8 first-run corruption ROOT-CAUSED + FIXED in RTL (stale `sc_burst_done` scale-burst skip) - VERIFIED ON SILICON

**2026-08-17 addendum (Q5 block-stream pipelining COMPLETE + VERIFIED ON SILICON):** the Q5_0 block-DMA path was re-pipelined to overlap DDR reads with core compute (see `docs/q5_q8_optimization_plan.md` Steps 3a/3b). `axihp_read_master.v` gained 1-deep pipelined-AR support (next burst's AR issues as soon as the AR channel frees; per-burst `done` preserved for single-flight Q8/CPU_OP callers). `hp_fsm_top.v` Q5 path now read-aheads block N+1 (gated on `!rd_start` so the block-1 start is never dropped), double-buffers the unpack registers, and dispatches strictly **in order** 0..55 (`q5_next_expected` guard - the Q5 core addresses activations by pulse-order `blk_counter`, so out-of-order pulses would multiply block N+2's weights against the wrong activation segment; the old A-first preference was exposed as `pblk=4` before `pblk=3` once reads ran ahead). Sim: 21 runs x 56 blocks strictly in-order; tb_hp_fsm_q5_0 10/10 + tb_hw_fsm_comprehensive 10/10 + tb_matmul_q8 6/6 + tb_q5_negd ALL PASS, no `$display` in RTL. Vivado: rebuilt via new `vivado_integration/improve_timing.tcl` (Performance_Explore + ExtraTimingOpt place + AggressiveExplore route + phys_opt) - WNS -0.343 ns / 6 failing endpoints (first pass -0.751/263), Slice LUTs 9,502, Slice 4,349 (98.84%). **NEW bitstream SHA256 `A5C1505757285D013FF42839EC310B1F222BAB0A8764B1AB37E162767596A848`, fused into BOOT.BIN SHA256 `ED866A0758C194E7F298F69044E51C55A6282150305591E233BFB7261B48286D`** (re-fused 2026-08-17; verified the embedded bitstream partition matches A5C15057). **VERIFIED ON SILICON:** `--compare` shows every `[q5 acc=gold]` and `[q8 acc=gold]` bit-exact, no `[q8 RAWDIFF]`, generation starts `9370 3837` (matches sim greedy reference). Q5 maxdiff 0.00013-0.0207 (S16 `d_pre` precision band); Q8 attn_v residual on blk.1/7/8/13 unchanged (open item below).

**2026-08-16 addendum (ROOT CAUSE FOUND - see Bug 3 at top):** Bug 3 (Q8 first-run corruption) is a **stale-`sc_burst_done` FSM handshake bug**, not uninitialized LUTRAM. The `INIT=0` LUTRAM change did not fix the board (correct - wrong theory). The bug was reproduced in sim by fixing `tb_hw_fsm_comprehensive.v` DDR model (4 MB -> 8 MB; Test 10 Q5 data at 0x00400000+ was previously out of bounds and the Q5 descriptor silently fell through to the Q8 dispatch), then the fix (`sc_burst_done <= 0;` in the Q8 FETCH_DESC dispatch) makes Test 10 pass. All sim suites pass 10/10. **Board follow-up (2026-08-17):** the non-ILA bitstream was rebuilt with the fix (SHA256 `37B472CC...`, see table below), BOOT.BIN re-fused (SHA256 `7540315F...`), deployed to SD, and the fix **VERIFIED ON SILICON**: `--compare` shows no `[q8 RAWDIFF]` (every Q8 `attn_v` bit-exact `acc=gold`), `--trace` layer norms match the sim (L0=17.50/17.49, L2=812.31/812.22), and generation starts `9370 3837` (matches the sim greedy reference). The now-redundant warm-up workaround + `--nowarmup` flag were removed from `linux/tmac_linux.c` and tmac rebuilt (md5 `cbe82673...`, deployed to SD).

**ILA diagnostic build (SUPERSEDED + REMOVED 2026-08-17):** the 142-bit `dbg_bus`/ILA build (bitstream `F1C4B07D...`, probe `linux/boot/system_wrapper_ila.ltx`) was created to observe the first Q8 COMPUTE on silicon; it is now **not needed** - the functional-sim root cause fully explains the board behavior. The ILA cell was dropped from `build_bd.tcl`, the non-ILA bitstream rebuilt with the fix, and `linux/boot/system_wrapper_ila.ltx` deleted. The `dbg_bus` port remains in `hp_fsm_top.v` (unconnected; optimized away by synthesis) in case the board re-verify disagrees with the sim and a future ILA is needed. Full ILA implementation/runtime/analysis write-up: `docs/debug_experiences.md`.

Open items: the data-dependent residual on `attn_v` blk.1/7/8/13 was confirmed on silicon (2026-08-17: blk.1=0.0135, blk.7=0.0131, blk.8=0.0109, blk.13=0.0157) - separate issue, likely Q8 scale/row-normalization precision.

**2026-08-14 addendum:** the long-standing "pre-existing Q5_0/Q8_0 precision issue" was root-caused and fixed - see **Key Fix (2026-08-14)** at the top. Three bugs: (1) Q5_0 `f16_decode` ignored the f16 sign bit (llama.cpp negative-d trick sign-flips ~50% of real blocks); (2) S48 accumulator cast to `int32_t` in board output scaling wrapped rows > 2^31; (3) first Q8 descriptor after a Q5_0 descriptor corrupts rows 16-63 (stale `sc_burst_done` from the Q5 path skips the first Q8 scale burst - fixed in RTL 2026-08-16, see Bug 3). Verified on silicon: Q5 maxdiff 0.0001-0.02, Q8 `attn_v` maxdiff 0.0004-0.004, no `[q8 RAWDIFF]`, layer norms match the sim.

**2026-08-13 addendum:** the two Linux Q6_K dequant bugs (missing block offset `bo`, and `f16_to_f32` unsigned-wrap on negative subnormals) were found and fixed �?see **Key Fix (2026-08-13)** at the top. Verified on hardware via on-board `--trace`: all 24 layer norms now match the sim exactly (the layer-0 115M explosion is gone).

**2026-08-12 update:** Three Q8 bugs found and fixed (see Key Fix section above): (1) address anticipation block optimized away by Vivado �?`pre_wmem_addr`/`pre_smem_addr`/`pre_act_addr` stuck at 0, causing all 512 compute iterations to read wmem[0]/smem[0]/act[0]; (2) DDR weight layout was column-major (`go[c*64 + r]`) but the FSM expected row-group-major (`(r>>3)*512 + c*8 + (r&7)`); (3) Linux scale values too small �?raw f16 block scales (d�?.001-0.1) gave UQ8.8 values of 0-25, making dequant �?0. Added row normalization. All three bugs were latent because **all existing tests used uniform data** (all-1s weights, scales, activations), which produces identical results regardless of address, layout, or scale. Fixed in RTL (`matmul_q8_core.v`), C++ test (`test_fpga_cores.cpp`), and Linux (`tmac_linux.c`).

| Resource | Used | Available | % | Notes |
|----------|------|-----------|---|-------|
| Slice LUTs | **9,346** | 17,600 | **53.10** | non-ILA rebuild 2026-08-17 w/ sc_burst_done fix (-22 vs ILA build) |
| LUT as Logic | **9,169** | 17,600 | **52.10** | |
| LUT as Memory | **177** | 6,000 | **2.95** | +80 from LUTRAM smem/act (was 97) |
| Slice Regs | 14,076 | 35,200 | 39.99 | |
| BRAM18 | **10** | 120 | **8.33** | Q8(8 wmem) + 2×Q5_0(1+1 act_mem) = 10 |
| DSP48E1 | **23** | 80 | **28.75** | |
| Slice | 4,303 | 4,400 | **97.80** | |
| **WNS** | **-0.209 ns** | 10 ns | 7 failing | non-ILA rebuild 2026-08-17; still works on HW |
| **SD boot** | **�?PASS** | �?| �?| FSBL→U-Boot→Linux verified (see #24) |

**Bitstream sources:** `axihp_read_master.v` + `axihp_write_master.v` + `matmul_q8_core.v` + `matmul_q5_0_core.v` + `hp_fsm_top.v`

**2026-08-17 bitstream (non-ILA, CURRENT):** Q5_0 `f16_decode` sign fix + Q8 power-up-init (INIT=0) + stale-`sc_burst_done` fix (`sc_burst_done <= 0;` in the Q8 dispatch). ILA cell dropped from `build_bd.tcl`. SHA256 `37B472CC1F6DD7539A1E0B4D464E97E3CA8E4E3C5F1AE05F93AE7FFA45507821` for `linux/boot/system_wrapper.bit`, fused into `BOOT.BIN` (SHA256 `7540315FD2D25C4913B131FEC9C5DEBD4BB11F8B5BA965D811832B39E91D44EB`, re-fused 2026-08-17; re-run `bootgen.bat` after any rebuild). The prior ILA diagnostic bitstream (`F1C4B07D...`, 2026-08-16) is removed. Deployed to SD + fix **VERIFIED ON SILICON** (no `[q8 RAWDIFF]`, layer norms match sim, generation starts `9370 3837`); warm-up + `--nowarmup` removed from `linux/tmac_linux.c`, tmac rebuilt (md5 `cbe82673...`) and deployed to SD. Final re-verify with the warm-up-free tmac is the remaining board step.
...
| 2 | Chain of 2 (1�?) | 1, 0 | 1, 1 | 229376, 0 | chain, clr_acc |
...
**Note (2026-08-12): All existing Q8 tests �?unit tests (`tb_cosim.v`, `tb_matmul_q8.v`), hardware tests (HP FSM comprehensive), and bare-metal tests �?used uniform weight/scale/act data. They could NOT detect the address advancement bug (all 512 compute iterations reading wmem[0]/smem[0]/act[0]) or the DDR weight layout mismatch (column-major vs row-group-major), because both bugs produce correct-looking results when all data values are identical. These bugs were fixed 2026-08-12 (see Key Fix section). Non-uniform HW verification completed 2026-08-13 (Q5 golden, Q8 row/col patterns, 14-group row-pattern �?see Key Fix 2026-08-13).**

```
test_fpga_cores.elf: 13 tests (indices 1-13, current source)
  - 1 Q8 all-1s:          PASS (2026-07-11, weights=1 acts=1 �?896/row)
  - 2 Q8 all(-1)s:        PASS (2026-07-11, signed path �?-896/row)
  - 3 Q8 distinct-sc:     PASS (2026-08-12, distinct block scales)
  - 4 Q8 row-weights:     PASS (2026-08-13, row-pattern �?64*(r+1), bit-exact after res_dout fix)
  - 5 Q5 val=1:           PASS (2026-07-11, �?229376/row)
  - 6 Q5 val=0:           PASS (2026-07-11, �?0/row)
  - 7 Q5 val=-1:          PASS (2026-07-11, �?-229376/row)
  - 8 Q8 col-weights:     PASS (2026-08-13, col-pattern �?2080/row)
  - 9 Q5 golden distinct: PASS (2026-08-13, bit-exact vs golden_model)
  - 10 14-group-col:      PASS (2026-08-13, row-pattern �?896*(r+1); earlier col-pattern fail was a TEST bug �?INT8 overflow)
...
2. �?Track A: smem→BRAM, act_reg→BRAM, wmem�?×BRAM (6-stage pipeline proven)
...
# FSBL build �?standard Vitis flow (two gating pitfalls below)
...
#              followed by `platform generate`. Vitis auto-creates a `zynq_fsbl` boot
...
#              workspace app �?XSCT fails with "project does not exist".
...
xsct.bat linux\build_fsbl.tcl       # XSA -> fsbl_platform -> zynq_fsbl.elf
# Optional: FSBL with debug banner:
xsct.bat linux\rebuild_fsbl_debug.tcl   # adds -DFSBL_DEBUG_INFO via Makefile CFLAGS

# BOOT.BIN fuse (run from linux/boot/)
bootgen.bat -image boot.bif -o BOOT.BIN -w
...
- `scripts/gguf_tok.py` �?pure-Python Qwen2 tokenizer (encode/decode/chat/run; no deps)
...
- `docs/debug_experiences.md` - Debugging toolbox: ILA implementation/runtime/analysis (first ILA campaign 2026-08-16), iVerilog sim discipline, golden models, host-side repros
- `docs/debug_procedures.md` - XSDB/DAP/ps7_init quick reference, AFI regs, FSM-hang decode
...
Three bugs fixed �?see **Key Decision #5** above for details (q8_wt_din unregistered, col_group reset in COMPUTE_W, act_remaining hardcoded). All three were found during iVerilog simulation of the comprehensive test suite.
...
| `tmac` | 469,060 B | Static ARM32 (mmaps FPGA at 0x43C00000); md5 cbe826732b9a1a400041f43c9882ae3d; warm-up workaround + `--nowarmup` removed 2026-08-17 (stale-`sc_burst_done` fix verified on silicon) |

**`tmac` CLI flags** (`./tmac model.tmac [flags] [token ...]`): `--cpu` force pure-CPU matmul path (skips FPGA init, so it can run host-side too); `--compare` run both CPU+FPGA per matmul and print diff; `--trace` per-layer hidden-state comparison (FPGA vs CPU, separate KV caches); `--selftest` minimal CPU_OP DDR copy (isolates PL DDR path from compute). `--trace` is the on-board A/B diagnostic used to verify the Q6_K dequant fix (layer norms vs sim ground truth). Multiple prompt tokens can be passed (e.g. `./tmac model.tmac 9707 0 2585 525 498 3351 30`) �?up to 256; default remains a bare prompt token (151646, a PAD token �?see below).

**Tokenizer tool (2026-08-14):** `scripts/gguf_tok.py` is a pure-Python (no `transformers`/`tiktoken`/`gguf` packages needed) Qwen2 tokenizer that reads the tokenizer metadata directly from the GGUF. Supports `--encode` (text→ids), `--decode` (ids→text), `--chat` (Qwen2 chat-template prompt→ids), and `--run` (run the host sim and print the generated words). **Critical: token 151646 is a PAD token (`[PAD151646]`), NOT BOS �?the GGUF says `bos_token_id=151643` (`<|endoftext|>`) with `add_bos_token=False`.** The old default prompt of `151646` therefore prompted the model with a padding token, which is why output was meaningless (`的，我就是我`). To get readable output, encode a real prompt with `--chat "..."` and pass the resulting ids to `tmac`/the sim.

**Generation protocol fix (2026-08-14):** `run_inference` in `tmac_linux.c` now matches the sim's `generate()`: it samples the first token from the *last prompt hidden state* (embedding BOS twice, at pos 0 then pos 1, caused a doubled-BOS offset vs the sim), then embeds the sampled token and forwards. Verified on host (x86 `gcc` build of `tmac_linux.c`, `--cpu` mode) �?output matches the sim's greedy single-BOS reference bit-for-bit (`9370 3837 35946 99486 35946 3837 35946 3837 35946 35946`). Board CPU `--cpu` output previously showed the doubled-BOS sequence (`9370 2073 854 33108 ...`), which matched the sim exactly when fed `[BOS, BOS]` as prompt �?confirming the forward pass was already bit-consistent. The `854, 854, ...` repetition is expected greedy-decoder behavior for a 0.5B model from a bare BOS, not a bug. Sim gained a `--greedy` flag (top_k=1) as the verification harness.
...
22. **U-Boot console fixed: DM serial bound to UART1 (dead) - DTS reroute to UART0 (2026-08-02):** Confirmed via embedded-DTB dump of the current build that the U-Boot driver-model console was bound to `serial@e0001000` (UART1, MIO 48/49) with UART0 (`serial@e0000000`) left `status="disabled"` �?so even after the zc702 DTB switch (#21) the banner never reached the CH340 on UART0 (MIO 14/15); only the hardcoded `debug_uart` (0xE0000000) ever emitted there. The earlier RAM-size fix (#21) was real and necessary (relocation landed at unpopulated 0x3FF00000), but the console was still routed to a dead UART1 afterward. Fix: in `arch/arm/dts/zynq-zc702.dts` (1) `aliases { serial0 = &uart1; }` -> `serial0 = &uart0;`, (2) replace the `&uart1 { u-boot,dm-pre-reloc; status="okay"; pinctrl uart1_10_grp; }` block with `&uart0 { u-boot,dm-pre-reloc; status="okay"; }` (no pinctrl needed �?ps7_init configures MIO 14/15, matching the `zynq-cc108.dts` UART0 pattern). The U-Boot serial driver probes UART0 successfully and debug_uart's 115200 config is preserved (probe early-returns if CR&TX_EN). Rebuilt: embedded DTB now shows UART0 okay + dm-pre-reloc, UART1 disabled (verified at ELF offset 0xc9690, 17192 bytes). Build scripts: `build_all.sh` + `build_uboot.sh` both mirror the patch via Python re.sub (idempotent). NOTE: the regex must consume the closing `};` not just `}` �?an early version produced `};;` which is a real dtc syntax error (found 2026-08-02 via `make dts`). New `vitis_linux/scripts/verify_uboot_console.tcl` loads u-boot.elf via JTAG (ps7_init PLL-guarded), runs 25 s so the terminal can show the banner + autoboot countdown, then dumps PC/regs/UART0. `debug_uboot_jtag.tcl` BSS_START updated to 0x040da760 (new build `__bss_start`). Helpers added in WSL: `dtb_find.py` (scans ELF for dtb magic + parses), `dts_serial.py` (prints serial0/status/pre-reloc). VERIFIED on HW 2026-08-02 via `verify_uboot_console.tcl`: full banner on CH340 at 115200 with `In/Out/Err: serial@e0000000`, `DRAM: 512 MiB`, `MMC: mmc@e0100000: 0` (no card present). U-Boot then falls through JTAG/QSPI/NAND/NOR boot attempts (expected - no media; GEM/USB/QSPI probe errors are cosmetic on this board). Next: U-Boot `mmc list`/`fatls mmc 0:1` SD test; then rebuild BOOT.BIN (new u-boot.elf) + BOOT.BIN SD boot.
