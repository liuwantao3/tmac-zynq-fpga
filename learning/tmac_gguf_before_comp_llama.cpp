...
float dequant_q5_0(const uint8_t* data, uint64_t idx) {
    ...
    uint8_t qs_byte = data[base + 6 + offset / 2];
    uint8_t ql = (offset % 2 == 0) ? (qs_byte & 0xF) : (qs_byte >> 4);
...
float dequant_q6_k(const uint8_t* data, uint64_t idx) {
    ...
    float d = f16_to_f32(d_raw);
    uint8_t ql_nibble = (offset % 2 == 0) ? (data[base + offset / 2] & 0xF) : (data[base + offset / 2] >> 4);
    uint8_t qh_nibble = (data[base + 128 + offset / 4] >> ((offset % 4) * 2)) & 0x3;
    int8_t q = ((qh_nibble << 4) | ql_nibble) - 32;
    uint8_t scale_idx = offset / 16;
    int8_t scale = data[base + 192 + scale_idx];
...
float read_embedding(const Tensor* t, uint64_t token_id, uint64_t feature) {
    // GGUF token_embd.weight shape: [hidden, vocab] = [896, 151936]
    // Data stored in row-major: [hidden][vocab]
    // For embedding[token_id][feature]: idx = feature * vocab_size + token_id
    // But t->cols = vocab_size = 151936
    uint64_t idx = feature * t->cols + token_id;
    if (t->type == TENSOR_Q8_0) return dequant_q8_0(t->data, idx);
    if (t->type == TENSOR_F32) return ((float*)t->data)[idx];
    if (t->type == TENSOR_F16) return f16_to_f32(((uint16_t*)t->data)[idx]);
    if (t->type == TENSOR_Q5_0) return dequant_q5_0(t->data, idx);
...
void rms_norm(float* o, const float* x, int n, const Tensor* t) {
    ...
    for (int i = 0; i < n; i++) o[i] = x[i] * w[i] / (r + 1e-5f);
}
...
void forward_layer_with_cache(float* hidden, int layer, int pos) {
    ...
    t = get_tensor_info(name);
    float k_new[K_DIM];
    matmul_transposed(t, attn_norm_out, k_new, K_DIM, HIDDEN_DIM);
    ...
    t = get_tensor_info(name);
    float v_new[V_DIM];
    matmul_transposed(t, attn_norm_out, v_new, V_DIM, HIDDEN_DIM);
    ...
    matmul_transposed(t, ffn_norm_out, gate, INTER_DIM, HIDDEN_DIM);
    ...
    t = get_tensor_info(name);
    float up[INTER_DIM];
    matmul_transposed(t, ffn_norm_out, up, INTER_DIM, HIDDEN_DIM);
    ...
    matmul_transposed(t, gate, ffn_out, HIDDEN_DIM, INTER_DIM);
...
void forward_layer(float* hidden, int layer, int pos) {
    ...
    t = get_tensor_info(name);
    float k[K_DIM];
    matmul_transposed(t, attn_norm_out, k, K_DIM, HIDDEN_DIM);
    snprintf(name, 128, "blk.%d.attn_k.bias", layer);
    t = get_tensor_info(name);
    ...
    t = get_tensor_info(name);
    float v[V_DIM];
    matmul_transposed(t, attn_norm_out, v, V_DIM, HIDDEN_DIM);
    snprintf(name, 128, "blk.%d.attn_v.bias", layer);
    t = get_tensor_info(name);
