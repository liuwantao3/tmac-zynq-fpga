...
int main() {
    ...
    // Negative d (llama.cpp negative-d trick: bit 15 set).
    CHECK(f16_decode_s2416(0xBC00) == -65536); // -1.0
    CHECK(f16_decode_s2416(0xB400) == -16384); // -0.25
    CHECK(f16_decode_s2416(0xB800) == -32768); // -0.5
    CHECK(f16_decode_s2416(0x8000) == 0);      // -0 (subnormal) -> 0
    ...
    // ---- Q5 tile negative d: all q5=+1, d=-0.25 (0xB400), norm=1.0, act=1 ----
    // d_pre = -16384, dq = -16384, sum over 896 = -14680064 (must NOT sign-flip to +).
    {
        uint8_t blocks[4*28*22]; uint16_t norm[4]; int16_t act[896]; int64_t out[4];
        for (int r = 0; r < 4; r++) for (int b = 0; b < 28; b++) {
            uint8_t* blk = blocks + (r*28+b)*22;
            blk[0] = 0x00; blk[1] = 0xB4;         // d = -0.25
            uint32_t qh = 0xFFFFFFFFu;
            blk[2] = qh & 0xFF; blk[3] = (qh>>8) & 0xFF;
            blk[4] = (qh>>16) & 0xFF; blk[5] = (qh>>24) & 0xFF;
            for (int i = 0; i < 16; i++) blk[6+i] = 0x11;  // q5 = +1
        }
        for (int i = 0; i < 4; i++) norm[i] = 256;
        for (int i = 0; i < 896; i++) act[i] = 1;
        q5_tile_golden(blocks, norm, act, out);
        for (int r = 0; r < 4; r++) CHECK(out[r] == -14680064);
    }
